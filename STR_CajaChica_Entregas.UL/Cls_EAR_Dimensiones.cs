...
{
    class Cls_EAR_Dimensiones : Cls_Global_Controles
    {
        ...
        //Ruta del formulario
        public const string gs_NomForm = "FormEARDIM";
        private string gs_RutaForm = "Resources/CajaChicaEAR/FrmDimDefault.srf";
        //DataSources
        private string gs_DTSDETHEMEAR2 = "@STR_HEMEAR2";
        //Columnas Matrix
        ...
        //* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
        //variables Globales....
        private string gs_CdgEmp = string.Empty;
        ...
        public void sb_FormLoad(string ps_Code)
        {
            try
            {
                ...
                go_Form.DataSources.DBDataSources.Add(gs_DTSDETHEMEAR2);
                go_Form.DataSources.UserDataSources.Add("UDS_IdEmp", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 10);
                go_Form.DataSources.UserDataSources.Item("UDS_IdEmp").Value = ps_Code;
                sb_SetColumnsMatrixDataBind();
        ...
        private void sb_SetColumnsMatrixDataBind()
        {
            go_Matrix = go_Form.Items.Item(gs_MtxDimns).Specific;
            go_Matrix.Columns.Item("clmDimNmb").DataBind.SetBound(true, gs_DTSDETHEMEAR2, gs_UflDetDimNmb);
            go_Matrix.Columns.Item("clmDimDsc").DataBind.SetBound(true, gs_DTSDETHEMEAR2, gs_UflDetDimDsc);
            go_Matrix.Columns.Item("clmValDfl").DataBind.SetBound(true, gs_DTSDETHEMEAR2, gs_UflDetDimDft);
        }
        ...
        private void sb_LoadMatrix(string ps_Code)
        {
            ...
            try
            {
                ...
                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).Query(lo_Cnds);
                ...
                if (go_Matrix.RowCount != li_ActvDmns)
                {
                    if (Cls_CCH_Crear_Accesos.go_DTblDimns != null)
                    {
                        ...
                        {
                            go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).Clear();
                            for (int i = 0; i < Cls_CCH_Crear_Accesos.go_DTblDimns.Rows.Count; i++)
                            {
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).InsertRecord(li_Fila);
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimNmb, li_Fila, Cls_CCH_Crear_Accesos.go_DTblDimns.Rows[i][1].ToString());
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimDsc, li_Fila, Cls_CCH_Crear_Accesos.go_DTblDimns.Rows[i][2].ToString());
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimDft, li_Fila, Cls_CCH_Crear_Accesos.go_DTblDimns.Rows[i][3].ToString());
                                li_Fila += 1;
                            }
                        }
                    }
                    else
                    {
                        ...
                        foreach (var lo_Item in lo_DmnsSrv.GetDimensionList())
                        {
                            ...
                            {
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).InsertRecord(li_Fila);
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimNmb, li_Fila, lo_Dim.DimensionName);
                                go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimDsc, li_Fila, lo_Dim.DimensionDescription);
                                li_Fila += 1;
                            }
                        }
                    }
                    ...
                    case SAPbouiCOM.BoFormItemTypes.it_BUTTON:
                        if (po_ItmEvnt.BeforeAction && po_ItmEvnt.ItemUID == "1")
                        {
                            if (go_Form.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                            {
                                ...
                                Cls_EAR_Crear_Accesos_BL.fn_AddDataFromDataSourceToDimnsTable(go_Form.DataSources.DBDataSources, go_Form.DataSources.UserDataSources.Item("UDS_IdEmp").Value);
                            }
                        }
                        break;
                }
        ...
        private bool fn_HandleChooseFromList(SAPbouiCOM.ItemEvent po_ItmEvnt)
        {
            ...
            ls_DimNmb = go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).GetValue(gs_UflDetDimNmb, lo_CFLEvnt.Row - 1).Trim();
            ...
            else
            {
                ...
                {
                    go_Form.DataSources.DBDataSources.Item(gs_DTSDETHEMEAR2).SetValue(gs_UflDetDimDft, lo_CFLEvnt.Row - 1, lo_DataTable.GetValue(0, 0));
                }
            }
