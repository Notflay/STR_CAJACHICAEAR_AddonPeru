...
namespace STR_CajaChica_Entregas.Metadata
{
    public class Cls_CCH_EAR_Init
    {
        ...
        public void sb_VerificarInstalacion()
        {
            ...
            {
                lo_UsrTbl = go_SBOCompany.UserTables.Item("STR_CCHEAR_SYS");
                ...
                else
                {
                    ...
                    {
                        lo_UsrTbl.GetByKey("001");
                        lo_UsrTbl.UserFields.Fields.Item("U_CE_ID").Value = li_IndInstal.ToString();
            ...
            if (lo_UsrTbl != null)
            {
                System.Runtime.InteropServices.Marshal.ReleaseComObject(lo_UsrTbl);
            }
            GC.Collect();
        ...
        private void sb_CreateUserTables()
        {
            ...
            try
            {
                GC.Collect();
                GC.WaitForPendingFinalizers();
                go_SBOApplication.StatusBar.SetText("CCH-EAR: Iniciando la creación de tablas de usuario...", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Success);
        ...
        private void sb_CreateUserFields()
        {
            ...
            try
            {
                GC.Collect();
                GC.WaitForPendingFinalizers();
                go_SBOApplication.StatusBar.SetText("CCH-EAR: Iniciando la creación de campos de usuario...", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Success);
                Cursor.Current = Cursors.WaitCursor;
        ...
        private void sb_CreateUserDefinedObjects()
        {
            ...
            try
            {
                ...
                go_SBOApplication.StatusBar.SetText("CCH-EAR: Iniciando la creación de objetos de usuario...", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Success);
                Cursor.Current = Cursors.WaitCursor;
                    ...
                    { 
                      if(Convert.ToInt32(lo_RecSet.Fields.Item(0).Value) != 0)
                      {
                          try
                          {
                              ...
                              {
                                  ls_StrFile = ls_StrFile.Replace("CREATE PROCEDURE", "CREATE PROC");
                                  ls_StrFile = ls_StrFile.Replace("CREATE PROC", "ALTER PROC");
                                  lo_RecSet.DoQuery(ls_StrFile);
                              }
